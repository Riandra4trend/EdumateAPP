...
export default function Home() {
  const { user, role, icon, signOut, fetchUserCourses, isUserCoursesUpdated } = useAuth();
  ...
  const handleForward = () => {
    if (courseIndex < userCourses.length - 1) {
    ...
    <View className="mx-4">
      <View className="w-82 h-20 bg-white rounded-lg mt-[22px] border border-neutral-300">
        <View className="flex-row py-4 px-4 justify-between">
          ...
          <TouchableOpacity
            className="flex-row items-center bg-[#F0CFF3] w-[100px] h-[40px] justify-center mr-3 rounded-full"
